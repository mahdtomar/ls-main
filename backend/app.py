...
app = Flask(__name__)
CORS(app) # Enables Cross-Origin Resource Sharing (CORS) so the frontend can call the backend.

# 🔹 Session Configuration (Added)
app.config["SECRET_KEY"] = "your_secure_random_secret_key"  # Change this to a strong random key
...
def login_required(role=None):
    """Decorator to protect routes, allowing only logged-in users with the correct role."""
    def wrapper(fn):
        @wraps(fn)
        def decorated_view(*args, **kwargs):
            ...
            if role and session.get('role') != role:
                return jsonify({'error': f'Access denied for {session.get("role")}'}), 403
...
@login_required('restaurant')  # Only restaurants can delete notifications
